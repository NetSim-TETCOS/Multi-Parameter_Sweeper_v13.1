...
for i in bandwidth:
	...
	for j in Tx_Antenna_count:
		for z in Rx_Antenna_count:
			...
			cmd='ConfigWriter.exe '+str(i)+' '+str(j)+' '+str(z)+' '+str(iat)+' '+str(prb_count[k-1])+' '+str(guard_band[k-1])
			print(cmd)
			os.system(cmd)
			...
			cmd="start \"NetSim_Multi_Parameter_Sweeper\" /wait /d "+ "\""+NETSIM_PATH+ "\" "\
			+"NetSimcore.exe -apppath \""+NETSIM_PATH+"\" -iopath \""+strIOPATH+\
			"\" -license "+"\"C:\\Program Files\\NetSim\\Pro_v13_1\\bin\""
			...
			if(os.path.isfile("Metrics.xml")):
				...
				csvfile = open("result.csv", 'a')
				csvfile.write('\n'+str(i)+','+str(j)+','+str(z)+','+str(iat)+','+'crash'+',')
				csvfile.close()

	    	#Name of the Output folder to which the results will be saved
			OUTPUT_PATH='Data\\'+str(foldername)+'\\Output_'+str(i)+'_'+str(j)+'_'+str(z);
